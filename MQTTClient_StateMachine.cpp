...
State::StateResult MQTTClient::Init_EventHandler(State::StateEvent* se)
{
    ...
    switch((int)se->evt)
    {
        case State::EV_ENTRY:
        {
        	DEBUG_TRACE_I(_EXPR_, _MODULE_, "Iniciando recuperaciÛn de datos...");
        	...
        	// realiza la suscripci√≥n local ej: "get.set/+/mqtt"
            ...
            if(MQ::MQClient::subscribe(subTopicLocal, &_subscriptionCb) == MQ::SUCCESS){
                DEBUG_TRACE_D(_EXPR_, _MODULE_, "Sucripci√≥n LOCAL hecha a %s", subTopicLocal);
            }
            else{
                DEBUG_TRACE_E(_EXPR_, _MODULE_, "ERR_SUBSC en la suscripci√≥n LOCAL a %s", subTopicLocal);
            }
            ...
            //carga la configuraci√≥n y ejecuta el cliente mqtt
        ...
        case MqttConnEvt:
        {
        	...
        	DEBUG_TRACE_D(_EXPR_, _MODULE_, "Iniciando subscripci√≥n a topics del servidor");
            int msgId;
            for(int i=0; i<MaxSubscribedTopics; i++)
            {
                msgId = esp_mqtt_client_subscribe(clientHandle, subscTopic[i], 1);
                DEBUG_TRACE_D(_EXPR_, _MODULE_, "Iniciando subscripci√≥n remota a topic: %s, msg_id=%d", subscTopic[i], msgId);
        ...
        case MqttSubscrEvt:
        {
            ...
            if(isTopicsSubscribed)
            {
                ...
                //comunicar a mqlib que el modulo est√° disponible
        ...
        case MqttDataEvt:
        {
			...
			if(strlen(localTopic) > 0){
                ...
                if(getRelativeTopic(relativeTopic, localTopic, &isOwnMsg))
                {
                    ...
                    if(blobData != NULL)
                    {
                        // Si el mensaje va dirigido unicamente a este nodo, se env√≠a a MQlib sin
                        // grupo y sin ID para ser procesado a nivel local. En caso de ir dirigido
                        // a m√°s nodos, se enviar√° tal cual para ser procesado por NetworkManager
                        if(isOwnMsg)
                        {
        ...
        case MqttPublishToServer:
        {
            ...
            if(isConnected)
            {
                ...
                {
                    DEBUG_TRACE_D(_EXPR_, _MODULE_, "El topic %s no est√° a√±adido en la lista de bridges de MQTTClient hacia el servidor", topicData->topic);
                }
			}
			else{
				DEBUG_TRACE_W(_EXPR_, _MODULE_, "ERR_FWD. No se puede enviar la publicaciÔøΩn, no hay conexiÔøΩn.");
			}
		...
		// Procesa datos recibidos de la publicaciÛn en cmd/$BASE/cfg/set
		case RecvCfgSet:{
			Blob::SetRequest_t<Blob::MQTTCfgData_t>* req = (Blob::SetRequest_t<Blob::MQTTCfgData_t>*)st_msg->msg;
			DEBUG_TRACE_I(_EXPR_, _MODULE_, "Recibida nueva configuraciÛn keys=%x", req->keys);
			// si no hay errores, actualiza la configuraciÛn
			if(req->_error.code == Blob::ErrOK){
				_updateConfig(req->data, req->keys, req->_error);
			}
			// si hay errores en el mensaje o en la actualizaciÛn, devuelve resultado sin hacer nada
			if(req->_error.code != Blob::ErrOK){
			...
			// si est· habilitada la notificaciÛn de actualizaciÛn, lo notifica
		...
		// Procesa datos recibidos de la publicaciÛn en cmd/$BASE/cfg/get
		case RecvCfgGet:{
			...
			// responde con los datos solicitados y con los errores (si hubiera) de la decodificaciÛn de la solicitud
			Blob::Response_t<Blob::MQTTCfgData_t>* resp = new Blob::Response_t<Blob::MQTTCfgData_t>(req->idTrans, req->_error, mqttLocalCfg);
			...
			// libera la memoria asignada al topic de publicaciÛn
			Heap::memFree(pub_topic);

			DEBUG_TRACE_I(_EXPR_, _MODULE_, "Enviada respuesta con la configuraciÛn solicitada");
			return State::HANDLED;
		}
